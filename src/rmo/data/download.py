"""Stage DeepFashion-MultiModal into ``data/raw`` and inventory what is there.
...
_MANIFEST_BEGIN = "<!-- BEGIN AUTOGENERATED MANIFEST -->"
_MANIFEST_END = "<!-- END AUTOGENERATED MANIFEST -->"
...
def render_manifest(records: Sequence[FileInfo], root: Path) -> str:
    """Render the inventory as a Markdown section for ``docs/DATASET.md``."""
    stamp = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC")
    lines = [
        _MANIFEST_BEGIN,
        "",
        f"_Generated by `scripts/download_data.py` on {stamp}. Re-run it to refresh._",
        "",
        f"Data root: `{root}`",
        "",
    ]
    ...
    if not records:
        lines += [
            "**Nothing staged yet.** Run:",
            "",
            "```",
            "python scripts/download_data.py --labels-only --from <unpacked-drive-dir>",
            "```",
            "",
            _MANIFEST_END,
        ]
    ...
    lines += [
        "| File | Size | Lines / entries | First record |",
        "|---|---|---|---|",
    ]
    for record in records:
        if record.json_entries is not None:
            count = f"{record.json_entries:,} ({record.json_kind})"
        elif record.lines is not None:
            count = f"{record.lines:,} lines"
        else:
            count = "—"
        sample = f"`{record.sample}`" if record.sample else "—"
        sample = sample.replace("|", "\\|")
        lines.append(f"| `{record.relpath}` | {record.size_human} | {count} | {sample} |")

    lines += ["", _MANIFEST_END]
    return "\n".join(lines)


def write_manifest(body: str) -> Path:
    """Write ``body`` into ``docs/DATASET.md``, replacing any previous generated block."""
    target = paths.ensure_dir(paths.docs_dir()) / "DATASET.md"

    if target.exists():
        existing = target.read_text(encoding="utf-8")
        if _MANIFEST_BEGIN in existing and _MANIFEST_END in existing:
            head, _, tail = existing.partition(_MANIFEST_BEGIN)
            _, _, tail = tail.partition(_MANIFEST_END)
        ...
        header = (
            ...
            "Non-commercial research use only; redistributing any portion of the\n"
            "images is forbidden, so `data/raw/**` is gitignored.\n"
        )
        merged = f"{header}\n{body}\n"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="download_data",
        description=(
            "Stage DeepFashion-MultiModal into data/raw and write an inventory "
            "report to docs/DATASET.md."
        ),
    ...
    parser.add_argument(
        "--verify",
        action="store_true",
        help="Skip staging entirely; just inventory data/raw and rewrite the "
        "manifest.",
    )
    parser.add_argument(
        "--dry-run",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    manifest = write_manifest(render_manifest(records, root))
    log.info("inventory: %d entries -> %s", len(records), manifest)
