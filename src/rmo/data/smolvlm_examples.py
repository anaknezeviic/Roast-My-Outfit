...
def build_training_example(image_id: str, raw_dir: Path) -> dict:
    """
    Build one SmolVLM training example from one image_id.
    """
    ...
    fabric_row = fabric.loc[
        fabric["image_id"] == image_id
    ].iloc[0]

    pattern_row = pattern.loc[
        pattern["image_id"] == image_id
    ].iloc[0]
